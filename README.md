...
Учебный проект для автоматизации принятия решений по кредитным заявкам: расчёт скоринга, REST API на FastAPI, пакетная обработка заявок, автотесты (включая проверки безопасности) и фабрика тестовых сущностей для ручной проверки.
...
Учебный проект курса по Python.
